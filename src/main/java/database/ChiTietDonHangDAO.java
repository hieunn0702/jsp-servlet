...
public class ChiTietDonHangDAO implements DAOInterface<ChiTietDonHang> {
		...
		try {
			...
			while (rs.next()) {
				...
				DonHang dh = new DonHangDAO().selectById(new DonHang(donhang, null, sql, donhang, maChiTietDonHang,
						donhang, masach, thuevat, tongtien, null, null));
				Sach s = new SachDAO().selectById(new Sach("", "", null, 0, 0, 0, 0, 0, null, "", ""));

				ChiTietDonHang ctdh = new ChiTietDonHang(maChiTietDonHang, dh, s, soluong, giagoc, giamgia, giaban,
						thuevat, tongtien);
				ketQua.add(ctdh);
			}
	...
	public ChiTietDonHang selectById(ChiTietDonHang t) {
		...
		try {
			...
			while (rs.next()) {
				...
				String maSach = rs.getString("maSach");// o
				int soluong = rs.getInt("soluong");
				double giagoc = rs.getDouble("giagoc");
				double giamgia = rs.getDouble("giamgia");
				double giaban = rs.getDouble("giaban");
				...
				DonHang dh = new DonHangDAO().selectById(new DonHang(donhang, null, sql, donhang, maChiTietDonHang,
						donhang, maSach, thuevat, tongtien, null, null));
				Sach sp = new SachDAO().selectById(new Sach(maSach, "", null, 0, 0, 0, 0, 0, null, "", ""));
	...
	public int insert(ChiTietDonHang t) {
		...
		try {
			...
			// Bước 2: tạo ra đối tượng statement
			String sql = "INSERT INTO chitietdonhang (machitietdonhang, donhang, masach, soluong, giagoc, giamgia, giaban, thuevat, tongtien) "
					+ " VALUES (?,?,?,?,?,?,?,?,?)";
	...
	public int update(ChiTietDonHang t) {
		...
		try {
			...
			// Bước 2: tạo ra đối tượng statement
			String sql = "UPDATE chitietdonhang SET donhang=?, masach=?, soluong=?, giagoc=?, giamgia=?, giaban=?, thuevat=?, tongtien=?"
					+ " WHERE machitietdonhang=?";

			PreparedStatement st = con.prepareStatement(sql);
			st.setString(1, t.getDonHang().getMaDonHang());
			st.setString(2, t.getSach().getMaSach());
